...
import com.pdsagame.backend.KnightsTour.service.KnightsTourService;
...
public class KnightsTourController {
    ...
    public Map<String, String> saveGameResult(@RequestBody Map<String, Object> request) {
        try {
            ...
            // ✅ Get algorithm times from request
            Integer warnsdorffTimeMs = null;
            Integer backtrackingTimeMs = null;

            Object warnTimeObj = request.get("warnsdorffTimeMs");
            Object backTimeObj = request.get("backtrackingTimeMs");

            if (warnTimeObj instanceof Integer) {
                warnsdorffTimeMs = (Integer) warnTimeObj;
            } else if (warnTimeObj instanceof Long) {
                warnsdorffTimeMs = ((Long) warnTimeObj).intValue();
            }

            if (backTimeObj instanceof Integer) {
                backtrackingTimeMs = (Integer) backTimeObj;
            } else if (backTimeObj instanceof Long) {
                backtrackingTimeMs = ((Long) backTimeObj).intValue();
            }
            ...
            knightsTourService.saveGameResult(
                    ...
                    warnsdorffTimeMs, backtrackingTimeMs
            );
